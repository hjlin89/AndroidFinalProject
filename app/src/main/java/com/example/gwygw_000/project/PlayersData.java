...
public class PlayersData {
    ...
    public PlayersData(String newstype){
	...
	public void initializeDataFromCloud(String name) {
        ...
        mRef.child("Players").child(name).addChildEventListener(new ChildEventListener() {
            @Override
...
//                }
    ...
    private void onItemAddedToCloud(HashMap<String, ?> item) {
        Long id = (Long) item.get("PlayerID");
        int insertPostion = 0;
        for (int i=0; i<newsList.size(); i++) {
            HashMap news = (HashMap) newsList.get(i);
            Long mid = (Long) news.get("PlayerID");
            if (mid == id) {
                return;
            }
